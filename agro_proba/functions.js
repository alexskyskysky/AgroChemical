...
{
	var rec = App.UploadGP.store.getById(id);
	rec.set(field, value);
	rec.commit();
};
...
{
    var selModel = btn.up('grid').getSelectionModel(), records;
	...
	App.MultiUploadFiles.abortUpload(records[0].getId());
};
...
var removeUpload = function (btn)
{
	...
	records = selModel.getSelection();
	App.MultiUploadFiles.removeUpload(records[0].getId());
};
